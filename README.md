...
- Some city names have some special characters, which is dependent on the author and language in which the city is located. However, some common words are mistaken as a city, for example, Nice, which can be defined as Nice, France by a geoparsing function. A way around this would be to look at the sentence in which the word was used. The world before and after may be good indicators for how that word is being referenced. 
